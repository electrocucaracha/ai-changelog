...
help:
	...
	@echo "  func  - Run functional Deepeval summarization tests"
...
func:
	...
	AI_CHANGELOG_DEEPEVAL_RUN=1 uv run --extra eval pytest tests/test_deepeval_summarization.py -m "integration and slow" -s
...
run:
	@if [ -z "$(REPO_PATH)" ]; then \
		...
		echo "Usage: make run REPO_PATH=/path/to/repo"; \
		exit 1; \
	fi
	uvx --from . ai-changelog $(REPO_PATH) $(ARGS)
